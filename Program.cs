...
namespace MaxTelegramBot
{
    class Program
    {
        ...
        // Таймеры ожидания кода авторизации по сессиям
        private static readonly Dictionary<string, CancellationTokenSource> _authTimeoutCtsBySession = new();
        ...
        private static void StartAuthorizationTimeout(long userId, long chatId, string sessionDir)
        {
            var cts = new CancellationTokenSource();
            _authTimeoutCtsBySession[sessionDir] = cts;
            ...
            _ = Task.Run(async () =>
            {
                try
                {
                    ...
                    if (_awaitingCodeSessionDirByUser.TryGetValue(userId, out var currentDir) && currentDir == sessionDir)
                    {
                    ...
                    _authTimeoutCtsBySession.Remove(sessionDir);
        ...
        {
            if (_awaitingCodeSessionDirByUser.TryGetValue(userId, out var sessionDir) &&
                _authTimeoutCtsBySession.TryGetValue(sessionDir, out var cts))
            {
